...
export class LayersService {
  ...
  generateTidesWmsUrl() {
    ...
    const dateTime = this.store.selectSnapshot(
      (state) => (state.tide as TideStateModel).unixTimestamp
    );
    this.tidesService.updateTideHeightFromApi(dateTime);
    let tideHeight = this.store.selectSnapshot(
      (state) => (state.tide as TideStateModel).tideHeight
    );

    let getMapRequest: string = `${this.geoServerRoot}/wms?service=WMS&version=1.1.0&request=GetMap&LAYERS=tidewalker:NIDEM_mosaic&SRS=epsg:3857&BBOX={bbox-epsg-3857}&WIDTH=256&HEIGHT=256&FORMAT=image/png&transparent=TRUE`;
  ...
  // generateTidesWmsLegendGraphicUrl() {
  //   // refresh the intertidal zone submergence WMS
  //   const dateTime = this.store.selectSnapshot(
  //     (state) => (state.tide as TideStateModel).unixTimestamp
  //   );
  ...
  //   let getLegendGraphicRequest: string = `${this.geoServerRoot}/wms?service=WMS&version=1.0.0&request=GetLegendGraphic&LAYER=tidewalker:NIDEM_mosaic&WIDTH=80&HEIGHT=80&FORMAT=image/png`;
  //   // `${this.geoServerRoot}/wms?service=WMS&version=1.3.0&request=GetMap&LAYERS=NIDEM&CRS=EPSG:3857&BBOX={bbox-epsg-3857}&WIDTH=256&HEIGHT=256&FORMAT=image/png&transparent=TRUE`;
  //   let sld_style: string = this.styleConstructor(tideHeight);
  //   let fullRequest: string = getLegendGraphicRequest + '&SLD_BODY=' + sld_style;
  //   console.log(fullRequest);
  //   this.store.dispatch(new TideActions.UpdateTideWmsLegendGraphicUrl(fullRequest));
